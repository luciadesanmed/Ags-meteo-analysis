...
n_vecinos = 5  
umbral_tolerancia = 3.0  # Diferencia máxima permitida en °C respecto al promedio local
...
valores_Tmean = gdf_temp['Tmean'].values
outliers_indices = []
...
scatter = gdf_final.plot(
    ...
    edgecolor='black',
    linewidth=0.6,
    legend_kwds={'label': "Temperature (°C)", 'orientation': "vertical", 'shrink': 0.7},
    zorder=3
)
...
ax_zoom.set_title('Annual mean temperature (CONAGUA, 1980-2025)', fontsize=16)
...
plt.tight_layout()
plt.savefig('Mapa_Temperatura_AGS.png', dpi=300, bbox_inches='tight')
plt.show()
